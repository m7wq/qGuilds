...
public class MySQL {
    ...
    public void removeGuild(String name){
        try (Connection connection = getConnection(); PreparedStatement statement= connection.prepareStatement("DELETE FROM guilds WHERE name = ?")){
            ...
            PluginProxied.getInstance().getGuildsManager().getGuildMap().remove(name);
        }catch (SQLException e){
            e.printStackTrace();
        }
    }
    ...
    public void createGuild(Member creator, Guild guild) {
        ...
        PluginProxied.getInstance().getGuildsManager().getGuildMap().put(guild.getGuildName(), guild);
        ...
        try (Connection connection = getConnection();
            ...
            statement.setString(4, guild.getGuildTag());
