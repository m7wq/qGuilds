...
public class SetTagColorSubCommand implements SubCommand {
    ...
    public void jobLoad(ProxiedPlayer sender, String... arg) {
        ...
        if (cooldown.getSetTagColorCooldown() != 0){
            sender.sendMessage(
        ...
        HashSet<String> canSetTagColor = new HashSet<>( configurationManager.getConfigures(ConfigurationManager.Configure.CAN_SET_TAG_COLOR));

        if (!canSetTagColor.contains(guildsManager.getPlayerRole(sender.getName()))){
        ...
        Plugin.getInstance().getMySQL().changeTagColor(tagColor,guildsManager.getPlayerGuild(sender.getName()).getGuildName());
