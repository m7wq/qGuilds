...
public class MuteSubCommand implements SubCommand {
    ...
    public void jobLoad(ProxiedPlayer sender, String... arg) {
        ...
        HashSet<String> canMute = new HashSet<>(cfg.getConfigures(ConfigurationManager.Configure.CAN_MUTE));

        if (!canMute.contains(guildsManager.getPlayerRole(sender.getName()))){
