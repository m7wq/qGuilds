...
public class LeaveSubCommand implements SubCommand {
    ...
    public void jobLoad(Player sender, String... arg) {
        if(!Plugin.getInstance().getGuildsManager().onGuild(sender.getName())){
            sender.sendMessage(Plugin.getInstance().getConfigurationManager().getMessage(sender,Message.ARENT_ON_GUILD_TO_LEAVE));
            return;
        
        }   

        if(Plugin.getInstance().getGuildsManager().getPlayerRole(sender.getName()).equalsIgnoreCase(RoleType.MASTER.name())){
            sender.sendMessage(Plugin.getInstance().getConfigurationManager().getMessage(sender,Message.YOU_ARE_MASTER));
            return;
        }

        Plugin.getInstance().getMembersManager().removePlayer(sender.getName());
        sender.sendMessage(Plugin.getInstance().getConfigurationManager().getMessage(sender,Message.YOU_LEFT_GUILD));
