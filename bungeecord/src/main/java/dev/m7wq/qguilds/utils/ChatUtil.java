...
public class ChatUtil {
    ...
    public static void checkCharactersAmount(ProxiedPlayer sender,String name, int amount){
        if (name.length()>amount){

            String msg = Plugin.getInstance().getConfigurationManager().getMessage(sender, ConfigurationManager.Message.TOO_LONG_NAME)
                    .replace("%int%",String.valueOf(amount));

            sender.sendMessage(
                    Plugin.getInstance().colorize(
                            msg
                    )
            );
        }
    ...
    public static void sendConfirming(String playerRole, String string, ProxiedPlayer sender){
        ...
        String highestRoleName = Plugin.getInstance().getGuildsManager().getPlayerGuild(sender.getName()).getRoles().get(0).getName();

        String command = playerRole.equalsIgnoreCase(highestRoleName) ? "/g disband" : "/g leave";
        String hoverText = playerRole.equalsIgnoreCase(highestRoleName) ? "&cDisband the guild" : "&cLeave the guild";
    ...
    public static void sendGuildMessage(ProxiedPlayer sender, String finalFormat, Guild playerGuild){
        ...
        String guildDisplayName = playerGuild.getGuildName();
        Role playerRole = guildsManager.getPlayerRole(playerName);
