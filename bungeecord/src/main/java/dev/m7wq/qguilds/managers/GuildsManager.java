...
import dev.m7wq.qguilds.data.MySQL;
...
public class GuildsManager{
    ...
    public void changeMapTag(String tag,String guildName){
        guildMap.get(guildName).setGuildTag(tag);
    }
    ...
    public @NotNull Guild getPlayerGuild(String ign){
       ...
       return new Guild(new ArrayList<>(),
        "", "", "", new ArrayList<>());
    }

    public @NotNull Role getPlayerRole(String ign){
        for (Guild guild : getGuildMap().values()){
            for (Member member : guild.getPlayersList()){
                if (member.getIgn().equalsIgnoreCase(ign))
