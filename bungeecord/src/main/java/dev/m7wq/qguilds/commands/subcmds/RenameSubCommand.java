...
public class RenameSubCommand implements SubCommand {
    ...
    public void jobLoad(ProxiedPlayer sender, String... arg) {
        ...
        if (cooldown.getRenameCooldown() != 0 && cd.isEnabled()){
            sender.sendMessage(
        ...
        HashSet<String> canRename = new HashSet<>(
                RolesUtil.getPermissionRoles(sender.getName(), Permission.can_rename)
        );

        if (!canRename.contains(guildsManager.getPlayerRole(sender.getName()).getName())){
        ...
        Guild guild = guildsManager.getPlayerGuild(sender.getName());

        if (guild.getOldName() == null){
            guild.setOldName(guild.getGuildName());
        }
        ...
        guildsManager.getGuildMap().remove(oldName);
