...
public class InviteSubCommand implements SubCommand {
    ...
    public void jobLoad(ProxiedPlayer sender, String... arg) {
        ...
        final Set<String> validRolesToInvite = new HashSet<>(
                RolesUtil.getPermissionRoles(sender.getName(), Permission.can_invite)
        );
        ...
        if (!validRolesToInvite.contains(manager.getPlayerRole(sender.getName()).getName())){
        ...
        for (String string : messagesConfig.getMessages(sender,ConfigurationManager.Message.INVITE_MESSAGE)){
            ...
            target.sendMessage(Plugin.getInstance().colorize(string.replace("%prefix%", Plugin.getInstance().getPrefix())
                    .replace("%inviter%",sender.getName())
                    .replace("%guild_name%",senderGuild.getGuildName())));
        }
        ...
        ProxyServer.getInstance().getScheduler().schedule(Plugin.getInstance(), new Runnable() {
            ...
            public void run() {
                        ...
                        if (invitingManager.getInvitedMap().get(targetName).contains(invitation)) {
                            ...
                            target.sendMessage(messagesConfig.getMessage(sender,ConfigurationManager.Message.EXPIRED_INVITE).replace("%guild_name%",senderGuild.getGuildName()));
                            invitingManager.getInvitedMap().get(targetName).remove(invitation);
                        }
